...
try {
    ...
    if (isModule) {
       xdmp.invoke(query, {options: options}, {database: xdmp.database(collectorDatabase), modules: xdmp.database(modulesDatabase) });
    } else {
        if (isMergingStep) {
        ...
        xdmp.eval(urisEval, {options: options}, {database: xdmp.database(collectorDatabase)});
    }
